...
function addToCart(goodsId) {
    ...
    $.ajax({
        type: "get",
        url: "/mobile/home/addToCart.do",
        cache: false,
        data: {id: goodsId,num:num,p:p},
        dataType: "json",
        success: function (data) {
            var jData = JSON.parse(data)
            if(jData.error=="no"){
                if( confirm("未登录，是否要登录？")){
                    window.location.href="/mobile/home/login.html"
                }
            }else{
            if (jData.error==0){
                alert("添加失败请重试！")
            }else{
                $("#total_number").html(jData.error)
            }}
        }
    })
...
function collect(goodsId) {
    ...
    $.ajax({
        type: "get",
        url: "/mobile/home/addCollection.do",
        cache: false,
        data: {id: goodsId,s:s},
        dataType: "json",
        success: function (data) {
            var jData = JSON.parse(data)
            collectResponse(jData);
        }
    })
}
